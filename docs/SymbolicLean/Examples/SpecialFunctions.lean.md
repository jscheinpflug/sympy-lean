...
## Public Surface
- Typechecking examples for trigonometric, logarithmic, square-root, exponential, complex-part, and variadic `Min` / `Max` pure heads.
- `#sympy` examples for exploratory special-function term construction.
- Executable examples covering effectful simplification / pretty-printing for rational and complex heads plus round-trip `reify` checks for `sin`, `atan2`, and `Max`.
